...
export default function PersonalView({ status, documentUrl, packageName, userName, userProfile }: PersonalViewProps) {
        ...
        if (status === 'pending_appointment' || status === 'paid') {
            return (
                <div className="min-h-[80vh] flex items-center justify-center p-6 animate-in fade-in duration-700">
                    <Card className="max-w-2xl w-full bg-[var(--dash-card)] rounded-[40px] shadow-2xl overflow-hidden border-[var(--dash-accent-border)]">
                        <div className="bg-[var(--dash-heading)] p-10 text-center relative overflow-hidden">
                            <div className="absolute inset-0 bg-[var(--dash-accent)] opacity-20 mix-blend-overlay"></div>
                            <Sparkles className="w-12 h-12 text-[var(--dash-muted-light)] mx-auto mb-4 animate-pulse" />
                            <h1 className="text-3xl md:text-4xl font-black text-white italic uppercase tracking-tighter mb-4">
                                Benvenuta, {userName || 'Atleta'}!
                            </h1>
                            <p className="text-[var(--dash-muted-light)] text-lg font-medium max-w-lg mx-auto">
                        ...
                        <div className="p-10 md:p-14 space-y-10">
                            <div className="text-center space-y-4">
                                <h2 className="text-2xl font-bold text-[var(--dash-heading)]">Il prossimo step: Conosciamoci!</h2>
                                <p className="text-[var(--dash-muted)] leading-relaxed text-lg">
                            ...
                            <div className="flex flex-col gap-4 max-w-sm mx-auto">
                                ...
                                <p className="text-center text-xs text-[var(--dash-muted-light)] font-medium">
                                    Ti risponderò al più presto per fissare l&apos;orario.
                                </p>
                            </div>

                            <div className="border-t border-[var(--dash-border)] pt-8 mt-8">
                                <div className="flex items-start gap-4 p-4 bg-[var(--dash-bg)] rounded-2xl border border-[var(--dash-accent-border)]">
                                    <div className="bg-[var(--dash-card)] p-2 rounded-full shadow-sm">
                                        <Calendar className="w-5 h-5 text-[var(--dash-accent)]" />
                                    </div>
                                    <div>
                                        <h4 className="font-bold text-[var(--dash-heading)] text-sm mb-1">Cosa succede dopo la call?</h4>
                                        <p className="text-xs text-[var(--dash-muted)] leading-normal">
        ...
        if (status === 'processing_plan') {
            return (
                <div className="w-full flex items-center justify-center animate-in fade-in duration-700">
                    <Card className="max-w-xl w-full bg-[var(--dash-card)] rounded-[32px] md:rounded-[40px] shadow-2xl p-6 md:p-10 text-center border-[var(--dash-accent-border)]">
                        <div className="w-14 h-14 md:w-20 md:h-20 bg-[var(--dash-card-header)] rounded-full flex items-center justify-center mx-auto mb-4 md:mb-6 relative">
                            <FileText className="w-7 h-7 md:w-10 md:h-10 text-[var(--dash-accent)] animate-pulse" />
                            <div className="absolute top-0 right-0 w-5 h-5 md:w-6 md:h-6 bg-[var(--dash-heading)] rounded-full flex items-center justify-center text-white text-[10px] md:text-xs font-bold animate-spin-slow">
                                ⚙️
                            </div>
                        </div>
                        <h2 className="text-xl md:text-3xl font-black text-[var(--dash-heading)] italic uppercase tracking-tighter mb-2 md:mb-4 leading-tight">
                            Ci stiamo lavorando!
                        </h2>
                        <p className="text-[var(--dash-muted)] text-sm md:text-lg mb-5 md:mb-8 leading-relaxed max-w-sm mx-auto">
                            Grazie per la call! Rita sta elaborando il tuo <strong>{packageName}</strong> basato su quanto vi siete dette.
                        </p>
                        <div className="inline-flex items-center gap-2 px-4 py-2 bg-[var(--dash-card-header)] rounded-full text-[var(--dash-accent)] text-xs md:text-sm font-bold">
            ...
            return (
                <div className="min-h-[80vh] flex flex-col items-center justify-center p-4 md:p-6 pb-24 animate-in fade-in duration-700 space-y-4 md:space-y-8">
                    <div className="text-center space-y-3">
                        ...
                        <h1 className="text-3xl md:text-6xl font-black text-[var(--dash-heading)] italic uppercase tracking-tighter leading-none shrink-0">
                            Il tuo Piano è Qui
                        </h1>
                    </div>

                    <Card className="max-w-4xl w-full bg-[var(--dash-card)] rounded-[32px] md:rounded-[40px] shadow-2xl overflow-hidden border-[var(--dash-accent-border)] flex flex-col md:flex-row">
                        <div className="bg-[var(--dash-heading)] p-8 md:p-10 md:w-2/5 flex flex-col justify-between relative overflow-hidden text-white min-h-[220px]">
                            <div className="absolute inset-0 bg-[url('/images/texture-noise.png')] opacity-10"></div>
                            <div className="relative z-10 flex flex-col items-center md:items-start text-center md:text-left">
                                <Sparkles className="w-8 h-8 text-[var(--dash-muted-light)] mb-4" />
                        ...
                        <div className="p-6 md:p-14 md:w-3/5 flex flex-col justify-center space-y-6 md:space-y-8">
                            <div className="text-center md:text-left">
                                <h4 className="text-xl md:text-2xl font-bold text-[var(--dash-text)] mb-2">Ora tocca a te!</h4>
                                <p className="text-[var(--dash-muted)] text-sm md:text-base leading-relaxed">
                            ...
                            <div className="space-y-4">
                                {documentUrl ? (
                                    <Button asChild className="w-full h-14 md:h-16 bg-[var(--dash-heading)] hover:bg-[var(--dash-heading)]/90 text-white rounded-xl md:rounded-2xl text-base md:text-lg font-bold shadow-xl shadow-[#593e25]/20 flex items-center justify-between px-6 md:px-8 hover:scale-[1.02] transition-all">
                                ...
                                <p className="text-center text-xs text-[var(--dash-muted-light)]">
                                    Hai bisogno di chiarimenti? <Link href="https://wa.me/393519398967" className="underline hover:text-[var(--dash-heading)] font-semibold">Scrivimi su WhatsApp</Link>
                                </p>
                            </div>
                        </div>
                    </Card>
    ...
    return (
        <div className="flex min-h-screen bg-[var(--dash-bg)] text-[var(--secondary)] selection:bg-brand/30 relative overflow-x-hidden w-full">
            <DashboardSidebar
                activeTab="1to1"
