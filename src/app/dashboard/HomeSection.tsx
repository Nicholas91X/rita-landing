...
export default function HomeSection({
    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {recentPackage && (
                <div className="relative overflow-hidden rounded-[32px] bg-[var(--dash-card)] border-2 border-[var(--dash-accent)] shadow-2xl flex flex-col md:flex-row">
                    {/* Main Ticket Section */}
                    <div className="flex-1 p-8 md:p-12 relative border-b-2 md:border-b-0 md:border-r-2 border-dashed border-[var(--dash-accent)]">
                        {/* Cutouts for ticket effect */}
                        <div className="absolute -left-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-[var(--dash-bg)] rounded-full border-2 border-[var(--dash-accent)]" />
                        <div className="absolute -right-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-[var(--dash-bg)] rounded-full border-2 border-[var(--dash-accent)] md:block hidden" />

                        <div className="space-y-6">
                            <div className="flex justify-between items-start">
                                <span className="text-[var(--dash-accent)] text-xs font-black uppercase tracking-[0.3em]">
                                    {userName ? `BOARDING PASS: ${userName}` : 'BOARDING PASS'}
                                </span>
                                <div className="text-[var(--dash-accent)] font-mono text-xs opacity-60">
                                    FLIGHT RW-2026
                                </div>
                            </div>

                            <div>
                                <h3 className="text-3xl md:text-4xl font-black text-[var(--dash-steel)] leading-tight italic uppercase tracking-tighter mb-2">
                                    Pronta per la tua prossima tappa?
                                </h3>
                                <p className="text-[var(--dash-text)]/70 text-lg font-medium">
                                    <span className="font-bold">✨ Prossima Destinazione:</span><br />
                                    {recentPackage.name}
                                </p>
                            </div>

                            <div className="flex gap-8 text-xs font-mono text-[var(--dash-accent)]/80">
                                <div>
                    ...
                    {/* Stub / Action Section */}
                    <div className="w-full md:w-64 bg-[var(--dash-card)] p-8 md:p-12 flex flex-col justify-center items-center gap-4 relative">
                        {/* Cutout for right side of stub */}
                        <div className="absolute -left-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-[var(--dash-bg)] rounded-full border-2 border-[var(--dash-accent)] md:block hidden" />
                        <div className="absolute -right-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-[var(--dash-bg)] rounded-full border-2 border-[var(--dash-accent)]" />
                        ...
                        <div className="w-full h-8 flex justify-between items-end opacity-20 mb-2">
                            {[...Array(20)].map((_, i) => {
                                ...
                                return (
                                    <div key={i} className="bg-[var(--dash-text)]" style={{
                                        width: `${pattern[i]}px`,
                                        height: heights[i]
                                    }} />
                                );
                        ...
                        <Button asChild size="lg" className="w-full bg-[var(--dash-steel)] hover:bg-[var(--dash-steel)]/90 text-[var(--dash-placeholder)] font-black py-6 rounded-xl shadow-lg shadow-black/5 group transition-all">
            ...
            <div className="bg-[var(--dash-card)] rounded-[32px] p-8 md:p-12 space-y-8 text-[var(--dash-text)] shadow-sm border border-[var(--dash-border)]">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                        <h4 className="text-xl font-bold text-[var(--dash-text)] tracking-tight">
                            Le tue precedenti destinazioni
                        </h4>
                        {allSubscriptionPackages.length > 0 && (
                            <span className="bg-[var(--dash-steel)]/10 text-[var(--dash-steel)] px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tight w-fit whitespace-nowrap">
                    ...
                    {allSubscriptionPackages.map((pkg) => {
                        return (
                            <Link
                                ...
                                className="p-6 bg-[var(--dash-card)] border border-[var(--dash-accent)] rounded-2xl hover:shadow-lg transition-all cursor-pointer group"
                            >
                                <div className="flex justify-between items-center">
                                    <div>
                                        <h5 className="text-[var(--dash-text)] font-bold">{pkg.name}</h5>
                                        <p className="text-[var(--dash-muted)] text-xs mt-1 uppercase tracking-widest font-bold">
                                            Vedi corso
                                        </p>
                                    </div>
                                    <div className="w-10 h-10 rounded-full bg-[var(--dash-placeholder)] flex items-center justify-center group-hover:bg-[var(--dash-placeholder)]/80 transition-colors">
                                        <ArrowRight className="w-5 h-5 text-[var(--dash-accent)] group-hover:scale-105 transition-transform" />
                                    </div>
                                </div>
                            </Link>
                        )
            ...
            {oneTimePurchases.length > 0 && (
                <div className="space-y-6">
                    <h4 className="text-xl md:text-2xl font-bold text-[var(--dash-heading)] tracking-tight uppercase">
                        ...
                        {oneTimePurchases.map((purchase) => {
                            ...
                            return (
                                <Link key={purchase.id} href={`/dashboard/package/${purchase.packages.id}?purchaseId=${purchase.id}`}>
                                    <div className="bg-[var(--dash-card)] rounded-[24px] overflow-hidden border border-[var(--dash-accent-border)] shadow-lg hover:shadow-xl transition-all duration-300 group flex flex-col h-full">
                                        <div className="relative h-48 shrink-0">
                                            ...
                                            <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-[10px] font-black uppercase text-[var(--dash-accent)] shadow-sm">
                                                {new Date(purchase.created_at).toLocaleDateString('it-IT')}
                                            </div>
                                        </div>
                                        <div className="p-6 flex justify-between items-center mt-auto">
                                            <div className="flex flex-col">
                                                <span className="text-[10px] text-[var(--dash-muted-light)] font-bold uppercase tracking-wider mb-0.5">Stato</span>
                                                <span className={`
                                            ...
                                            <div className="w-10 h-10 rounded-full bg-[var(--dash-placeholder)] flex items-center justify-center group-hover:bg-[var(--dash-accent)] group-hover:text-white transition-colors shadow-sm">
